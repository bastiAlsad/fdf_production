# Generated by Django 5.0.6 on 2024-05-29 19:50

import datetime
import django.db.models.deletion
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='AiExercise',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('exercise_id', models.IntegerField(default=0)),
            ],
        ),
        migrations.CreateModel(
            name='BillingCount',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('billing_count', models.IntegerField(default=1)),
            ],
        ),
        migrations.CreateModel(
            name='ChatDataModel',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('is_sender', models.BooleanField(default=False)),
                ('message', models.CharField(default='', max_length=2003000000)),
                ('created_by', models.IntegerField(default=0)),
                ('admin_id', models.IntegerField(default=0)),
            ],
        ),
        migrations.CreateModel(
            name='Coupon',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('coupon_name', models.CharField(default='', max_length=200)),
                ('coupon_text', models.CharField(default='', max_length=400)),
                ('coupon_image_url', models.CharField(default='', max_length=200)),
                ('coupon_price', models.IntegerField(default=0)),
                ('coupon_offer', models.CharField(default='', max_length=100)),
                ('rabbat_code', models.CharField(default='', max_length=250)),
            ],
        ),
        migrations.CreateModel(
            name='DoneExercises',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('exercise_id', models.IntegerField(default=0)),
                ('laxout_user_id', models.IntegerField(default=0)),
                ('date', models.DateTimeField(default=datetime.datetime(2024, 5, 29, 21, 50, 31, 903324))),
            ],
        ),
        migrations.CreateModel(
            name='DoneWorkouts',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('workout_id', models.IntegerField(default=0)),
                ('laxout_user_id', models.IntegerField(default=0)),
                ('date', models.DateTimeField(default=datetime.datetime(2024, 5, 29, 21, 50, 31, 903323))),
            ],
        ),
        migrations.CreateModel(
            name='First',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('first', models.IntegerField(default=0)),
            ],
        ),
        migrations.CreateModel(
            name='IndexesLaxoutUser',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('index', models.IntegerField(default=0)),
                ('creation_date', models.IntegerField(default=5)),
                ('created_by', models.IntegerField(blank=True, default=None)),
            ],
        ),
        migrations.CreateModel(
            name='Laxout_Exercise',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('execution', models.CharField(default='', max_length=10000)),
                ('name', models.CharField(default='', max_length=40)),
                ('dauer', models.IntegerField(default=30)),
                ('videoPath', models.CharField(default='', max_length=100)),
                ('looping', models.BooleanField(default=False)),
                ('added', models.BooleanField(default=False)),
                ('instruction', models.CharField(default='', max_length=200)),
                ('timer', models.BooleanField(default=False)),
                ('required', models.CharField(default='', max_length=50)),
                ('imagePath', models.CharField(default='', max_length=50)),
                ('appId', models.IntegerField(default=0)),
                ('onlineVideoPath', models.CharField(default='', max_length=220)),
            ],
        ),
        migrations.CreateModel(
            name='Laxout_Exercise_Order_For_User',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('laxout_user_id', models.IntegerField(default=0)),
                ('laxout_exercise_id', models.IntegerField(default=0)),
                ('order', models.IntegerField(default=0)),
            ],
        ),
        migrations.CreateModel(
            name='LaxoutUserIndexCreationLog',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('for_month', models.IntegerField(default=5)),
                ('for_year', models.IntegerField(default=2024)),
                ('created_by', models.IntegerField(blank=True, default=None)),
                ('for_week', models.IntegerField(default=22)),
                ('related_user_pain', models.IntegerField(default=0)),
            ],
        ),
        migrations.CreateModel(
            name='LaxoutUserPains',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('for_month', models.IntegerField(default=5)),
                ('created_by', models.IntegerField(blank=True, default=None)),
                ('for_year', models.IntegerField(default=2024)),
                ('zero_two', models.IntegerField(default=0)),
                ('theree_five', models.IntegerField(default=0)),
                ('six_eight', models.IntegerField(default=0)),
                ('nine_ten', models.IntegerField(default=0)),
                ('for_week', models.IntegerField(default=22)),
                ('admin_id', models.IntegerField(blank=True, default=0)),
            ],
        ),
        migrations.CreateModel(
            name='LaxTree',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('condition', models.IntegerField(default=0)),
            ],
        ),
        migrations.CreateModel(
            name='Second',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('second', models.IntegerField(default=0)),
            ],
        ),
        migrations.CreateModel(
            name='SkippedExercises',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('skipped_exercise_id', models.IntegerField(default=0)),
                ('laxout_user_id', models.IntegerField(default=0)),
            ],
        ),
        migrations.CreateModel(
            name='SuccessControll',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('better', models.BooleanField(default=False)),
                ('created_by', models.IntegerField(default=0)),
            ],
        ),
        migrations.CreateModel(
            name='WebCodes',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('created_by', models.IntegerField(default=0)),
                ('code', models.CharField(default='', max_length=20)),
            ],
        ),
        migrations.CreateModel(
            name='AiTrainingData',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('illness', models.CharField(default='', max_length=200)),
                ('created_by', models.IntegerField(default=0)),
                ('created_for', models.IntegerField(default=0)),
                ('related_exercises', models.ManyToManyField(to='fdf_app.aiexercise')),
            ],
        ),
        migrations.CreateModel(
            name='Kursprogramm',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(default='', max_length=290)),
                ('programm_number', models.CharField(default='', max_length=200)),
                ('note', models.CharField(default='', max_length=200)),
                ('creation_date', models.DateField(default=datetime.datetime(2024, 5, 29, 19, 50, 31, 909324, tzinfo=datetime.timezone.utc))),
                ('instruction', models.CharField(default='', max_length=200)),
                ('instruction_in_int', models.IntegerField(default=1)),
                ('created_by', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
                ('exercises', models.ManyToManyField(to='fdf_app.laxout_exercise')),
            ],
        ),
        migrations.CreateModel(
            name='LaxoutUser',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('user_uid', models.CharField(default='', max_length=420)),
                ('laxout_user_name', models.CharField(default='', max_length=200)),
                ('laxout_credits', models.IntegerField(default=0)),
                ('last_login', models.DateTimeField(default=datetime.datetime(2023, 11, 14, 0, 0))),
                ('last_login_2', models.DateTimeField(default=datetime.datetime(2023, 11, 14, 0, 0))),
                ('last_meet', models.DateField(default=datetime.datetime(2024, 5, 29, 21, 50, 31, 905324))),
                ('lax_tree_id', models.IntegerField(default=0)),
                ('water_drops_count', models.IntegerField(default=0)),
                ('email_adress', models.CharField(default='', max_length=40)),
                ('admin_has_seen_chat', models.BooleanField(default=True)),
                ('user_has_seen_chat', models.BooleanField(default=False)),
                ('was_created_through_app', models.BooleanField(default=True)),
                ('belongs_to_programm_number', models.CharField(default='', max_length=200)),
                ('creation_date', models.DateField(default=datetime.datetime(2024, 5, 29, 19, 50, 31, 905323, tzinfo=datetime.timezone.utc))),
                ('instruction', models.CharField(default='', max_length=200)),
                ('coupons', models.ManyToManyField(to='fdf_app.coupon')),
                ('created_by', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
                ('exercises', models.ManyToManyField(to='fdf_app.laxout_exercise')),
                ('indexes', models.ManyToManyField(to='fdf_app.indexeslaxoutuser')),
            ],
        ),
        migrations.CreateModel(
            name='Uebungen_Models',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('execution', models.CharField(default='', max_length=10000)),
                ('name', models.CharField(default='', max_length=40)),
                ('dauer', models.IntegerField(default=30)),
                ('videoPath', models.CharField(default='', max_length=100)),
                ('looping', models.BooleanField(default=False)),
                ('added', models.BooleanField(default=False)),
                ('instruction', models.CharField(default='', max_length=200)),
                ('timer', models.BooleanField(default=False)),
                ('required', models.CharField(default='', max_length=50)),
                ('imagePath', models.CharField(default='', max_length=50)),
                ('appId', models.IntegerField(default=0)),
                ('onlineVideoPath', models.CharField(default='', max_length=220)),
                ('first', models.ManyToManyField(to='fdf_app.first')),
                ('second', models.ManyToManyField(to='fdf_app.second')),
            ],
        ),
        migrations.CreateModel(
            name='UserProfile',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('user', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
        ),
    ]
